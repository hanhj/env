...
install :apt install libcurl4-openssl-dev
...
可用来查询计算机的位数:getconf LONG_BIT. Windows中用dxdiag来查看操作系统版本。
...
- 集线器原理：  
	集线器工作在物理层（phy）。仅仅起到信号放大的作用，很少的软件，基本上是一个硬件设备。  
	当某个端口收到数据，其将信号放大并广播到其他所有端口，而不像交换机一样具有记忆mac地址功能可以点对点的通信，降低了带宽。  

- 交换机原理：  
	交换机工作在链路层。记录每个端口的Mac地址，当不同主机需要交换数据时，由交换机根据mac地址进行点对点的数据交换，从而避免了冲突避让。

![](net_device.png)

- 路由器原理：  
	路由器工作在网络层（IP协议）。用于连接两个不同网段的机器。一般路由器有多个Lan口和1个Wan口，Lan口和Wan口的网段不同。比如到Wan口的地址为192.168.0.102，子网掩码为255.255.255.0，则其所在网段IP地址为192.168.0.xxx  
	Lan口的地址为：192.168.1.1，子网掩码为255.255.255.0，则Lan口的网段IP地址为192.168.1.xxx。   
![](net_device2.png)   
	路由器通过子网掩码可以区分不同网段，在路由器中有路由表，当某个Lan口收到数据时，如果不是在本网段内，则默认的将该数据发送到Wan口，从而实现Lan口的机器访问Wan口机器或者上互联网。  
	为了能够从外部访问到Lan口内部的机器（主动的），需要在Wan口所在的网段的路由器（即本路由器的上一级路由器）的路由表中添加一条路由信息，将访问lan口的要求路由到本路由器。
如下图所示：  
![](net_device3.png)  
![](net_device4.png)
